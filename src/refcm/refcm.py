...
class RefCM:
    def __init__(
        ...
        cache_fpath: str | None = None
            Path to stored mapping costs. Defaults to system cache directory.
    ...
    def setref(self, ref: AnnData, ref_id: AnnData, ref_key: str) -> None:
        ...
        for i, label in enumerate(labels):
            self.ref_ltk[label], self.ref_ktl[i] = i, label
    ...
    def annotate(
        ...
        for i, label in enumerate(clusters):
            self.q.obs.loc[self.q.obs[self.q_key] == label, "refcm_clusters"] = i
    ...
    def lp_match(self, cost: np.ndarray) -> np.ndarray:
        ...
        log.debug("starting LP optimization")
    ...
    def _cache_get(self) -> List[List[float]] | None:
        ...
        if len(result) > 1:
            log.error("Corrupted Cache: conflicting entries found.")
    ...
    def _update_cache(self, mcosts: List[List[float]]) -> None:
        ...
        os.makedirs(os.path.dirname(self._cache_fpath), exist_ok=True)
