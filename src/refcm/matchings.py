...
class Matching:
    def __init__(
        ...
        * ideally, ground truth labels follow the same clustering as the one that
        ...
        if not os.path.exists(config.TREE_FILE):
            os.makedirs(os.path.dirname(config.TREE_FILE), exist_ok=True)
            default_tree = files("refcm").joinpath("typetree.json")
            shutil.copy(default_tree, config.TREE_FILE)
    ...
    def set_type_equality_strictness(self, t: float) -> None:
        if t < 0 or t > 1:
            logging.error("Type strictness must be between 0 and 1.")
        self.strictness = t
